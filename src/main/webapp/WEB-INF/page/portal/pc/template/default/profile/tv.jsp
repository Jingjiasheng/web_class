...
<style>
	...
	.p-drama-grid>li{max-width: 77px; height: 45px; line-height: 45px; font-size: 18px; background: #f7f7fc; text-align: center; margin-right: 10px; margin-bottom: 10px;}
	.p-drama-grid>li a{display: block;}
	.p-drama-grid>li a:hover{color: #2fb3ff;}
					...
					<div class="main">
						<div class="tab">
							...
							<div class="tab-content">
								<ul id="play-list" class="equal-10 p-drama-grid" style="width: 880px;">
...
<script>
			...
			success : function(rtn) {
				...
				if (list.length>0) {
					...
					for (var i=0; i<list.length; i++) {
						html += '<li>';
						html += '<a href="play.action?videoId='+list[i].videoId+'" target="_blank">'+list[i].num+'</a>';
						html += '</li>';
					}
	...
	function getMonthRank() {
		$.ajax({
			url : path + "api/rank.json?apiId=49",
			type : "POST",
			dataType : "json",
			success : function(rtn) {
				if (rtn.code=="999999") {
