...
li.j_block{width: 51px; height: 51px; overflow: hidden; margin-bottom: 10px;}
li.j_block a{position: relative; background: #282828; display: block; width: 49px; height: 49px; line-height: 49px; text-align: center; border: 1px solid rgba(255,255,255,.1); color: #999; font-size: 16px; border-radius: 4px;}
li.j_block.selected a{color: #2fb3ff; border: 1px solid #2fb3ff;}
li.j_block a:hover{color:#fff;}
			...
			<div class="main-0">
				<div style="display: flex;">
					...
					<div class="play-list">
						...
						<ul id="play-list" class="equal-5 play-list-ul">
							...
							<div style="color: #666;position: relative;">
								...
								<div class="fengge">
									...
									<c:forEach items="${mediaInfo.tv_leixing}" var="value" varStatus="status">
										<c:choose>
											<c:when test="${(status.index+1)==fn:length(mediaInfo.tv_leixing)}">
												<a href="${pageContext.request.contextPath}/portal/list.action?typeId=3&name=类型&value=${value}" target="_blank" class="tag-link-item">${value}</a>
											</c:when>
											<c:otherwise>
												<a href="${pageContext.request.contextPath}/portal/list.action?typeId=3&name=类型&value=${value}" target="_blank" class="tag-link-item">${value}</a>
				...
				if (list.length>0) {
					...
					for (var i=0; i<list.length; i++) {
						if (videoId==list[i].videoId) {
							html += '<li id="li_video_'+videoId+'" class="j_block selected">';
						} else {
							html += '<li class="j_block">';
						}
						if (videoId==list[i].videoId) {
							html += '<a href="play.action?videoId='+list[i].videoId+'">'+list[i].num+'</a>';
						} else {
							html += '<a href="play.action?videoId='+list[i].videoId+'">'+list[i].num+'</a>';
						}
						html += '</li>';
