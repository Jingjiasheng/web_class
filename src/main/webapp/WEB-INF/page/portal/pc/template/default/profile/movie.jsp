...
<style>
	...
	.p-drama-grid>li{height: 45px; line-height: 45px; font-size: 18px; background: #f7f7fc; margin-bottom: 10px; padding-left: 20px;}
					...
					<div class="main">
						<div class="tab">
							...
							<div class="tab-content">
								<ul id="play-list" class="p-drama-grid" style="width: 880px;">
...
<script>
			...
			success : function(rtn) {
				...
				if (list.length>0) {
					...
					for (var i=0; i<list.length; i++) {
						html += '<li>';
						if (!list[i].title) {
							html += '<a href="play.action?videoId='+list[i].videoId+'" target="_blank">'+biaoti+'</a>';
						} else {
							html += '<a href="play.action?videoId='+list[i].videoId+'" target="_blank">'+list[i].title+'</a>';
						}
						html += '</li>';
					}
	...
	function getMonthRank() {
		$.ajax({
			url : path + "api/rank.json?apiId=52",
			type : "POST",
			dataType : "json",
			success : function(rtn) {
				if (rtn.code=="999999") {
